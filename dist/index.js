...
(async () => {
  const rules = await (0, readPrompts_1.readPrompts)(".promptRules");
  // builds all possible prompts
  const filePrompts = await (0, buildPromptsFromRules_1.buildPromptsFromRules)(
    rules
  );
  // removes the prompts that have already been processed
  const incompleteFilePrompts = (0,
  removeGeneratedFilePrompts_1.removeGeneratedFilePrompts)(filePrompts);
  const failedFiles = await (0,
  iterativelyGenerateFilesFromPrompts_1.iterativelyGenerateFilesFromPrompts)(
